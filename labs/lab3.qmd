...
**a)** Create a histogram plot of the ages of our discussion partners in the following code chunk. Use the `ggplot()` and `geom_histogram` commands. Fill in the blanks below.
...
In the above code, we use the pipe `%>%` to pass the `responses` dataframe into the next function.
...
`distinct(ego_id, ego_ai)`: This keeps only one row per ego based on `ego_id`. If the same `ego_id` appears multiple times, duplicates are removed and only the first occurrence is kept. After this step, the dataset contains one row per ego, that is, 15 rows in total since there are 15 egos.
...
**2.1**: Fill in the blanks in the code below to make a plot of the rates of unauthorized AI use among students in the class and among their UT student alters.
...
ego_and_alter_ai_response %>% 
  ggplot(aes(x = ________, y = _________, fill = __________)) + 
  geom_col() + 
