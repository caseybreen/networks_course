...
We will use the Florentine Family Marriage dataset as a running example for network analysis. Before we start, we need to install and load required packages. A package is a collection of functions (e.g., functions to plot networks) that will help us with our analysis.
...
You can find more informationabout how this dataset was generated from this paper:
...
-   It is undirected (marriage is mutual). If person A is married to person B, person B is married to person A.
...
`count(name = "n_nodes")` counts the number of rows and assigns the number to "n_nodes".
...
1.2 What would happen if we remove Medici from the network? Run the code below and describe how the network has changed. Now which family appears to be the most important? Is the network more fragmented?

1.3 Based on your answers to 1.1 and 1.2, describe two types of social capital that the Medici family likely has. How do you know?
...
1.4. Run the code below. Based on the number of priors, which family appears the most important? Based on wealth? What does a network perspective add? 
...
Now we'll turn to analyzing data from our homophily survey. In this survey, we collected information about an ego and five of their discussion partners (alters). We dropped nonsensical responses and missing data. We'll read this file in from where it is stored (on Github) using the `read_csv()` function.   
...
First, let's look at the average age of our discussion partners. To do this, we will need to use the `summarize()` function with the `mean()` function. The `summarise()` function collapses a table into a summary and produces one row with summary statistics (e.g, `mean()` or `median()`).
...
Does average age of our discussion partners vary by gender? Here, we can use the `group_by(ego_gender)` function to calculate the mean age of alters by gender.
...
We can also look at the age of the younger discussion partner and the oldest partner, using the `summarize()` function along with `min()` and `max().`
...
gender_tabulated %>% 
  ...
  geom_tile() +
  geom_text(aes(label = n), size = 7, color = "black") +
...
Next, we visualize this information using ggplot. The function `ggplot()` function initializes a plot using the summarized dataset, which we pass to it using the ` %>% ` operator. Inside `aes()` (aesthetic mapping), we specify how variables should appear in the visualization. Here, `alter_gender` is mapped to the x-axis, `ego_gender` to the y-axis, and the variable `n` (the count of ties) to the fill color.
...
`geom_text(aes(label = n), size = 4)`: the function `geom_text()` adds text labels to each tile. Inside aes(), we specify that the label should display the value of `n`, which shows the exact number of ties in each cell.
...
```
Wow! Nobody here listed a Freshman as one of their discussion partners. 
...
Assume that men and women are equally represented in the our class. In a world with no homophily and approximately equal numbers of men and women, what percentage of ties would we expect to be cross-gender (using $2pq$)? How does this compare to what we observed? Is this evidence of homophily? 
...
2.5. Revisit our survey data. Use the `filter()` function and the `alter_age` variable to restrict the data to discussion partners older than 40. Among these ties, what percentage are cross-gender ties (men → women or women → men)? Does this look different from the percentage of cross-gender ties when we use all discussion partners without filtering by age? Does this pattern surprise you, and why or why not?

Create a new code chunk to do this. 
